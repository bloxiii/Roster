...
async function seedAgency(agency: DemoAgencyConfig) {
  ...
  if (existingCompany) {
    ...
    const { error: updateError } = await supabase
      .from("companies")
      // status: "active" — les nouveaux comptes créés via handle_new_user()
      // démarrent probablement en attente de validation manuelle (comportement
      // prévu pour les vrais clients) ; ces comptes démo doivent être actifs
      // immédiatement pour que /api/agent/chat accepte les requêtes du widget.
      .update({
        slug: agency.slug,
        name: agency.name,
