...
export function ProspectDetail({
  ...
  const date = new Date(prospect.createdAt).toLocaleDateString("fr-FR", {
    day: "numeric",
    month: "long",
    year: "numeric",
    hour: "2-digit",
    ...
    <div>
      ...
      <div className="mt-6 flex items-start justify-between gap-4">
        ...
        <span
          className={`inline-flex rounded-full border px-3 py-1 font-mono text-[10px] uppercase tracking-widest ${
            QUAL_STYLES[d.qualification]
          }`}
        >
          {QUAL_LABELS[d.qualification]}
        </span>
      </div>
      ...
      {prospect.notifiedAt && (
        <div className="mt-4 flex items-center gap-2 rounded-lg bg-status/10 px-3 py-2 text-xs text-status">
          ...
          {new Date(prospect.notifiedAt).toLocaleDateString("fr-FR", {
            day: "numeric",
            month: "long",
            hour: "2-digit",
            minute: "2-digit",
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_1fr]">
        ...
        <div>
          ...
          {d.resume && (
            <div className="mt-4 rounded-xl border border-border bg-surface p-5">
              <h3 className="font-mono text-[10px] uppercase tracking-widest text-paper-dim/60">
                Résumé
              </h3>
              <p className="mt-2 text-sm leading-relaxed text-paper-dim">{d.resume}</p>
            </div>
          )}

          {d.notes_commerciales && (
            <div className="mt-4 rounded-xl border border-brass/30 bg-brass/5 p-5">
              <h3 className="font-mono text-[10px] uppercase tracking-widest text-brass">
        ...
        <div>
          <h2 className="font-mono text-xs uppercase tracking-widest text-paper-dim/60">
            Conversation ({prospect.conversationLength} messages)
          </h2>
          <div className="mt-4 rounded-xl border border-border bg-surface p-4">
            {prospect.conversation.length === 0 ? (
              ...
              <div className="space-y-3">
                {prospect.conversation.map((msg, i) => (
                  <div
                    key={i}
